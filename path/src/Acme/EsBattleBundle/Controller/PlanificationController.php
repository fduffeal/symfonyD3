...
//		$response->setSharedMaxAge(300);
        ...
        if (!$collection) {
            $query = $em->createQuery(
                'SELECT planification
            FROM AcmeEsBattleBundle:Planification planification
            WHERE planification.isDefault = true '
            )->setMaxResults(1);
    ...
    public function listAction(Request $request, $offset, $limit)
    {
        ...
        $arrayIdCurrentPartenaire = array();
    ...
    public function createAction(Request $request)
    {
        ...
        if ($form->isValid()) {
            ...
            if (empty ($otherPlanification)) {
                $this->_keepOnlyOneDefault($planification);
                $em = $this->getDoctrine()->getManager();
    ...
    {
        $dateStart = ($planification->getStart() !== null)?$planification->getStart():new \DateTime();
        $dateEnd = ($planification->getEnd() !== null)?$planification->getEnd():new \DateTime("+4 hours");
        ...
        if ($user->isModo() == true) {
            $formBuilder->add('isDefault', 'checkbox', array(
                'required' => false
            ));
        }
    ...
    /**
     * @var \Acme\EsBattleBundle\Entity\Planification $currentPlanification ;
     */
    private function _keepOnlyOneDefault($currentPlanification)
    {

        if ($currentPlanification->getIsDefault() !== true) {
            return;
        }
        $em = $this->getDoctrine()->getManager();
        ...
        $query = $em->createQuery(
            ...
            WHERE planification.isDefault = TRUE
            AND planification.id != :currentId'
        )->setParameter('currentId', $currentPlanification->getId());
         ...
         */
        foreach ($collection as $planification) {
            $planification->setIsDefault(false);
        }
        $em->flush();
    }
